...
class AdminController extends Controller{
    ...
    function __construct(){
        ...
        if(!empty($admin_name)){
            //A.处于登录状态
            if($admin_name!=='admin'){
                $cc = "Category-getCatByPid";
                //dump($cc);
                $nowAC = $nowAC1.",".$cc;
                //1) 只有普通管理员需要访问权限限制
                $roleinfo = D('Role')->find($admin_role);
                $roleAC1 = $roleinfo['role_auth_ac'];//Goods-showlist,Order-tianjia
                //dump($roleAC1);
                $roleAC = $roleAC1.",".$cc;
                //dump($roleAC);
                ...
                $allowAC = "Manager-login,Manager-verifyImg,Manager-logout,Manager-changepassword,Userbaojia-getuserbaojia,Yuming-getyuming,Index-top,Index-index,Index-left,Index-right,Index-down,Index-center,Yuming-find3,Category-getCatByPid";
